...
if (!window.ErrorHandler) {
    class ErrorHandler {
        ...
        async sendErrorToServer(errorLog) {
            if (!navigator.onLine) {
                console.warn('無法發送錯誤，因為設備處於離線狀態。');
                return;
            }

            // 開發環境下禁用伺服器日誌記錄，避免產生錯誤
    // 修改：增加對 file:// 協議的檢查，避免 CORS 錯誤
    if (window.location.hostname === 'localhost' || 
        window.location.hostname === '127.0.0.1' || 
        window.location.protocol === 'file:') {
        console.log('本地環境: 錯誤日誌僅在本地記錄');
        ...
        return;
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`伺服器返回錯誤狀態碼: ${response.status}`);
                    this.storeErrorInLocalStorage(errorLog);
                } else {
                    console.log('錯誤已成功發送到伺服器。');
                }
            } catch (e) {
                console.warn('發送錯誤到伺服器失敗 - 將在本地存儲:', e);
                this.storeErrorInLocalStorage(errorLog);
            }
        }
